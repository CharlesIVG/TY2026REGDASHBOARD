...
(function () {
  ...
  function renderDaily() {
    ...
    set("rep-total", today.total);
    for (const k of ORDER) set("rep-" + k, today[k] || 0);
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      week.push({
        ...
        isFuture: false,
        // the day is only a confirmed zero if a record exists AND it was
        // actually sampled that day (rec.hasData); a missing record or a
        // carried-forward one means we simply don't know
        hasData: !!rec && rec.hasData !== false
      });
    }

    const weekTotal = week.reduce((a, w) => a + w.total, 0);
    const peak = Math.max(0, ...week.map(w => w.total));
