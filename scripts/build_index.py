...
(function () {
  ...
  .oc-repnote{font-family:'IBM Plex Mono',monospace;font-size:9px;color:var(--muted);opacity:.75;margin-top:6px;line-height:1.4;}
  /* Team-size composition. A mean was meaningless (you cannot have a
     third of a person), so this shows the actual tally at a size people
     can read across the room. Team count sits top-right, clear of it. */
  .oc-repcomp{margin-top:11px;padding-top:9px;border-top:1px solid var(--line);}
  .oc-repcomph{display:flex;align-items:baseline;justify-content:space-between;gap:12px;}
  .oc-repcomph .l{font-family:'IBM Plex Mono',monospace;font-size:10px;color:var(--muted);
  ...
  const I18N = {
    en: { dark:"Dark", light:"Light", totalTeams:"Total Teams", view:"View ▸", all:"All",
          ...
          why_goal_reached:"Green: campaign goal reached",
  ...
  async function poll() {
    try {
      ...
      /* "4=76  3=20  2=29" - largest teams first, sizes read from the data
         rather than hardcoded, so a 5-person team would appear on its own.
         Values are coerced to Number before going anywhere near innerHTML. */
      const teamSizeTally = d => !d ? [] : Object.keys(d)
        .sort((a, b) => Number(b) - Number(a))
        .filter(k => d[k])
        .map(k => Number(k) + "=" + Number(d[k]));
      if (teamsize && teamsize.people) {
        ...
        if (parts.length) {
          ROOT.querySelector("#rep-comp-row").style.display = "";
          ROOT.querySelector("#rep-comp-l").textContent = t("compTitle");
          ROOT.querySelector("#rep-comp-teams").textContent = t("teamsCount")(teamsize.teams);
          ROOT.querySelector("#rep-comp").innerHTML =
            parts.map(p => "<span>" + p + "</span>").join("");
        }
        ROOT.querySelector("#rep-people-note").textContent = t("peopleNote")(teamsize.asOf);
      }
      paint();
      renderDaily();
      renderWeekly();
